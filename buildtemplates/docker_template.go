...
const DockerTemplate = `FROM ubuntu:18.04
...
RUN apt-get update && \
    apt-get -y install bison build-essential bzip2 ccache curl flex gcc-multilib git\
...
if [ 1 -eq "${do_quick_check-0}" ] ; then
  if ! missing_packages="$(dpkg-query -W -f ' ' ${packages} 2>&1)"; then
    ...
    if [ -n "${unknown}" ]; then
      ...
      echo "(maybe missing a repo or need to ' apt-get update'):"
...
# Fix deps
 apt-get -f install
# common
 apt-get -y install lib32z1 lighttpd python-pexpect xvfb x11-utils
